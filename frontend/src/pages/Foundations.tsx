...
const Swatch: React.FC<{ name: string; bg: string; caption: string }> = ({ name, bg, caption }) => (
  <Stack spacing={1}>
    <Box h={14} bg={bg} borderRadius="card" border="1px" borderColor="border.subdue" />
            ...
            <SimpleGrid columns={{ base: 2, md: 4, lg: 6 }} spacing={4}>
              {flatten(group as SemanticGroup, family).map(([name, value]) => (
                <Swatch
                  ...
                  name={name}
                  bg={name}
                  caption={colorMode === 'light' ? value.default : value._dark}
                />
              ))}
            </SimpleGrid>
          </Section>
          ...
          <Stack spacing={4}>
            {Object.entries(primitives).map(([hue, steps]) => (
              <HStack key={hue} spacing={4} align="flex-start">
                <Text textStyle="bodyMdEmphasis" w={24} pt={2}>
                  {hue}
                </Text>
          ...
          <Stack spacing={3}>
            {Object.keys(textStyles).map((name) => (
              <HStack key={name} spacing={4} align="baseline">
                <Text textStyle="bodySm" color="text.subdue" w={44} flexShrink={0}>
                  {name}
                </Text>
          ...
          <HStack spacing={6} align="flex-end">
            {Object.entries(space).map(([key, px]) => (
              <Stack key={key} spacing={1} align="center">
                {/* `w={key}` would resolve against theme.sizes (Chakra's default sizes['2xs'] is 16rem), so use the value. */}
                <Box w={px} h={px} bg="bg.info" border="1px" borderColor="border.regular" />
                <Text textStyle="bodyXs" color="text.subdue">
                  {key} · {px}
                </Text>
              </Stack>
